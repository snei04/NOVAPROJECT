...
export class BoardsService {
  ...
  getActivity(boardId: Board['id']) {
    return this.http.get<Activity[]>(`${this.apiUrl}/api/v1/boards/${boardId}/activity`, {
      context: checkToken(),
    });
  }
   addMember(boardId: string, email: string) {
    return this.http.post(`${this.apiUrl}/api/v1/boards/${boardId}/members`, { email }, {
      context: checkToken(),
    });
  }
}
